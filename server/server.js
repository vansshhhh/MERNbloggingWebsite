...
import serviceAccountKey  from "./mern-blogging-website-b9bb3-firebase-adminsdk-fizqw-487294dc58.json" assert{type:"json"};
...
server.post("/change-password", verifyJWT, (req,res) => {    
    ...
    .then((user) => {
        if(user.google_auth){
            return res.status(403).json({error : "You cannot change account's password beacuse you are logged-in through Google."})
        }

        bcrypt.compare(currentPassword, user.personal_info.password, (err, result) => {
            if(err){
                return res.status(500).json({error: "Some error occured while changing the password. Plese try again later!"})
            }
            if(!result){
            ...
            bcrypt.hash(newPassword, 10, (err, hashed_password) => {
                ...
                .catch(err => {
                    return res.status(500).json({error : "Some error occured while saaving the new password. Please try agin later!"})
                })
            })
        })
    })
        ...
        for(let i = 0; i < socialLinksArr.length; i++){
            if(social_links[socialLinksArr[i]].length){
                ...
                if(!hostname.includes(`${socialLinksArr[i]}.com`) && socialLinksArr[i] != "website"){
                    return res.status(403).json({error: `${socialLinksArr[i]} link is invalid.  You mut enter a full link.`})
                }
...
server.post("/create-blog", verifyJWT, (req,res) => {
    ...
    if(!draft){
        ...
        if(tags.length > 10){
            return res.status(403).json({"error":"There should not be more than 10 tags."})
        }
    }
    ...
    if(id){
        Blog.findOneAndUpdate({blog_id}, {title, des,banner, content, draft: draft ? draft : false})
        .then(blog => {
